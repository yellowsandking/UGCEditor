...
2. 点击 **「生成 DSL」**。
3. 生成结果会写入页面上的「② DSL（JSON）」文本框（并同步文档状态）；**不会在项目文件夹里自动新建 `.json` 文件**。
4. 若要把 DSL 存成磁盘上的文件，请到「② DSL（JSON）」区域点击 **「下载 JSON 文件」**（默认文件名为 `ugc-scene.json`，由浏览器保存到你选择的目录）。
...
- **「下载 JSON 文件」**：将**当前文档**（与可视化编辑一致）导出为 `ugc-scene.json`，通过浏览器下载保存到本地；这是获得磁盘上 JSON 文件的正式方式。
